import { ROOM_DIMENSIONS } from '../../utils/pos';
...
const findPositionsInsideRect = (rect: Rect): Coord[] => {
  const positions = [];
  ...
  for (let x = rect.x1; x <= rect.x2; x++) {
    for (let y = rect.y1; y <= rect.y2; y++) {
      ...
      if (x < 0 || x >= ROOM_DIMENSIONS || y < 0 || y >= ROOM_DIMENSIONS) continue;
...
export const floodFill = (room: Room, seeds: Coord[], enableVisuals?: boolean) => {
