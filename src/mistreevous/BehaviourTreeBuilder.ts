...
export default function buildRootNode(definition: RootNodeDefinition[], options: BehaviourTreeOptions): Root {
    ...
    const rootNode = nodeFactory(rootNodeDefinitionMap[MAIN_ROOT_NODE_KEY], rootNodeDefinitionMap, options, "root") as Root;
...
function nodeFactory(
    ...
    const child = (suffix: string) => (def: AnyNodeDefinition) =>
        nodeFactory(def, rootNodeDefinitionMap, options, path + suffix);

    // Create the node instance based on the definition type.
    let node: AnyNode;
    switch (definition.type) {
        case "root":
            node = new Root(attributes, options, child(".c")(definition.child));
            break;
        ...
        case "flip":
            node = new Flip(attributes, options, child(".c")(definition.child));
            break;

        case "succeed":
            node = new Succeed(attributes, options, child(".c")(definition.child));
            break;

        case "fail":
            node = new Fail(attributes, options, child(".c")(definition.child));
            break;
        ...
        case "sequence":
            node = new Sequence(
                attributes,
                options,
                definition.children.map((c, i) => child("." + i)(c))
            );
            break;
        ...
        case "branch":
            return nodeFactory(rootNodeDefinitionMap[definition.ref].child, rootNodeDefinitionMap, options, path);

        case "action":
            node = new Action(attributes, options, definition.call, definition.args || []);
            break;

        case "condition":
            node = new Condition(attributes, options, definition.call, definition.args || []);
            break;
        ...
        case "wait": {
            ...
            node = new Wait(attributes, options, duration, durationMin, durationMax);
            break;
        }
    }

    // Assign the deterministic path as the node uid so memory keys are stable across global resets.
    (node as any).uid = path;
