...
export default abstract class Node {
    /**
     * The node unique identifier. Assigned a deterministic path string by the builder when a memory store is used.
     */
    ...
    /**
     * The node state. Only used when no memory store is provided via options.
     */
    ...
    /**
     * Gets/Sets the state of the node. Reads/writes from options.memory when available.
     */
    getState = (): AnyState => this.options.memory ? (this.options.memory[this.uid] ?? State.READY) : this._state;
    setState = (value: AnyState): void => {
        const previousState = this.getState();

        if (this.options.memory) {
            this.options.memory[this.uid] = value;
        } else {
            this._state = value;
        }
